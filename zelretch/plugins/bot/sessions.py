...
@zelretch.bot.on_message(filters.regex(r"📟 Session") & Config.AUTH_USERS & filters.private)
...
async def _create_new_session(message: Message):
    await message.reply_text(
        "**Okay.** Let's set up a new userbot session.",
        reply_markup=ReplyKeyboardRemove(),
    )

    phone_number = await zelretch.bot.ask(
        message.chat.id,
        "**1.** Enter the Telegram account phone number for the new session.\n\n"
        "Use international format, for example: `+8801XXXXXXXXX`\n\n"
        "Send /cancel to stop this setup.",
    ...
    try:
        ...
        ask_otp = await zelretch.bot.ask(
            message.chat.id,
            "**2.** Enter the OTP sent by Telegram.\n\n"
        ...
        await message.reply_text(
            "**Success.** Session string generated. Adding it to the database..."
        )
        user_id = (await client.get_me()).id
        await db.update_session(user_id, session_string)
        await message.reply_text(
            "**Success.** Session added to the database. Restart the bot to load this account.\n\n"
            "**Security note:** the session string is saved internally and will not be shown in chat."
        )
    except TimeoutError:
        await message.reply_text(
            "**Timeout.** You took too long to complete the setup. Please try again."
        )
...
@zelretch.bot.on_message(filters.regex(r"New 💫") & Config.AUTH_USERS & filters.private)
...
async def _send_delete_session(message: Message, *, edit: bool = False):
    ...
    if not all_sessions:
        text = "No sessions found in the database."
        if edit:
    ...
    text = "**Choose a session to delete:**"
    if edit:
...
async def rm_session_cb(client: Client, cb: CallbackQuery):
    ...
    if cb.from_user.id not in [user_id, owner_id]:
        return await cb.answer(
            f"Access restricted. Only {owner_name} and the session owner can delete this session.",
            show_alert=True,
        )

    await db.rm_session(user_id)
    await cb.answer("Session deleted. Restart the bot to apply changes.", show_alert=True)
...
async def _send_session_list(message: Message, *, edit: bool = False):
    ...
    text = f"**{Symbols.cross_mark} Session List**\n\n"
    for i, session in enumerate(all_sessions, start=1):
        text += f"[{i:02d}] {Symbols.bullet} **User ID:** `{session['user_id']}`\n"
...
@zelretch.bot.on_message(filters.regex(r"List 📜") & Config.AUTH_USERS & filters.private)
...
@zelretch.bot.on_message(filters.regex(r"Home 🏠") & filters.private & Config.AUTH_USERS)
...
BotHelp("Sessions").add(
    "session", "Open the session manager to create, delete, or list userbot sessions."
).info(
    "Session 🚀"
).done()
