...
async def stanUsers(client: Client, message: Message):
    ...
    if not users:
        return await zelretch.delete(hell, "__No stans found!__")
...
async def addstan(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        return await zelretch.delete(message, "__I can't be a stan of myself!__")

    if await db.is_stan(client.me.id, user.id):
        return await zelretch.delete(message, "__This user is already a stan!__")

    await db.add_stan(client.me.id, user.id)
    await zelretch.delete(message, f"__Added {user.mention} as a stan!__")
...
async def delstan(client: Client, message: Message):
    ...
    if await db.is_stan(client.me.id, user.id):
        ...
        await zelretch.delete(message, f"__Removed {user.mention} from stans!__")

        Config.AUTH_USERS.remove(user.id)
        Config.STAN_USERS.remove(user.id)
    else:
        await zelretch.delete(message, "__This user is not a stan!__")
