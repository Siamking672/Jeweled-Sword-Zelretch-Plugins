...
async def youtube_audio(_, message: Message):
    ...
    hell = await zelretch.edit(message, "Checking ...")
    status, url = YoutubeDriver.check_url(query)

    if not status:
        return await zelretch.delete(hell, url)
    ...
    try:
        ...
        return await zelretch.delete(hell, f"**🍀 Audio not Downloaded:** `{e}`")
...
@on_message("ytv", allow_master=True)
async def ytvideo(_, message: Message):
    ...
    try:
        ...
        return await zelretch.delete(hell, f"**🍀 Video not Downloaded:** `{e}`")
...
@on_message("ytlink", allow_master=True)
async def ytlink(_, message: Message):
    if len(message.command) < 2:
        return await zelretch.delete(message, "Give something to search on youtube.")
    ...
    try:
        ...
        return await zelretch.delete(hell, f"**🍀 Error:** `{e}`")
