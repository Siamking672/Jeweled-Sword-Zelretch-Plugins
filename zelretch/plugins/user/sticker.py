...
async def kangSticker(client: Client, message: Message):
    ...
    hell = await zelretch.edit(message, "__Kanging sticker...__")
    ...
    if pack_type is None:
        return await zelretch.delete(hell, "Unsupported media type.")
    ...
    if message.reply_to_message.sticker:
        if is_static:
            ...
            if not status:
                return await zelretch.error(hell, path)
        else:
    ...
    try:
        ...
        return await zelretch.delete(
            hell, f"Start @{zelretch.bot.me.username} first and try to kang again.", 20
        )
...
async def packKang(client: Client, message: Message):
    ...
    hell = await zelretch.edit(message, "__Kanging sticker pack...__")
    ...
    if not message.reply_to_message.sticker:
        return await zelretch.delete(hell, "Reply to a sticker to kang whole pack!")
...
async def stickerInfo(_, message: Message):
    ...
    hell = await zelretch.edit(message, "__Fetching sticker info ...__")

    sticker = message.reply_to_message.sticker

    sticker_set = await get_sticker_set(zelretch.bot, sticker.set_name)
    if not sticker_set:
        return await zelretch.delete(hell, "This sticker is not part of a pack.")
...
async def removeSticker(_, message: Message):
    ...
    hell = await zelretch.edit(message, "__Removing sticker from pack...__")
