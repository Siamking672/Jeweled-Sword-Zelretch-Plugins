...
async def _AuthUsers() -> None:
    ...
    temp_list.extend([(await client.get_me()).id for client in zelretch.users])

    master_users = await db.get_all_masters()
    ...
    temp_list = None
    LOGS.info(
        f"{Symbols.arrow_right * 2} Registered Authorized Magi {Symbols.arrow_left * 2}"
    )


async def _MasterUsers() -> None:
    users = await db.get_all_masters()
    for user in users:
...
async def _GbanUsers() -> None:
    ...
    LOGS.info(
        f"{Symbols.arrow_right * 2} Added {len(users)} Sealed Targets {Symbols.arrow_left * 2}"
    )
    ...
    LOGS.info(
        f"{Symbols.arrow_right * 2} Added {len(musers)} Silenced Targets {Symbols.arrow_left * 2}"
    )


async def UserSetup() -> None:
    """Initialize Users Config"""
    LOGS.info(f"{Symbols.bullet * 3} Preparing Master Registry {Symbols.bullet * 3}")
    await _AuthUsers()
    await _MasterUsers()
    await _GbanUsers()
...
async def TemplateSetup() -> None:
    ...
    with open("zelretch/functions/templates.py", "r", encoding="utf-8") as file:
