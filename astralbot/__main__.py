...
async def main() -> int:
    ...
    # 5. Pyrogram clients
    try:
        clients = await build_clients(config, db)
    except Exception as exc:
    ...
    # 6. Bind globals (so plugins' `from astralbot import ...` works)
    primary = clients[0]
...
async def pyrogram_idle() -> None:
    """Replacement for pyrogram.idle() that doesn't depend on internals."""
