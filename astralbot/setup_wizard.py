...
<label>Log chat ID (optional)</label>
<input type="text" name="log_chat_id" placeholder="-1001234567890">
<div class="hint">Telegram chat ID for WARNING+ log forwarding.</div>
...
<div class="hint">Telegram will send a login code to this number via SMS or Telegram app.</div>

<button type="submit" name="action" value="request_code" class="secondary">Send login code</button>
...
async def _send_code(api_id: int, api_hash: str, phone: str) -> dict:
    ...
    from pyrogram.errors import PhoneInvalid, PhoneNumberInvalid, FloodWait
    ...
    try:
        ...
        return {"ok": True}
    except (PhoneInvalid, PhoneNumberInvalid) as exc:
...
def _render_page2() -> str:
    """Render page 2 with current state (info/error/code/2FA blocks)."""
    info_block = ""
    if _STATE.get("info"):
    ...
    code_block = ""
    if _STATE.get("phone") and not _STATE.get("session_string"):
        code_block = """
        <label>Login code</label>
        <input type="text" name="code" placeholder="12345" autocomplete="off">
        <button type="submit" name="action" value="submit_code" class="secondary">Verify code</button>
        """

    password_block = ""
    if _STATE.get("2fa_needed") and not _STATE.get("session_string"):
        password_block = """
        <label>2FA password</label>
        <input type="password" name="password" placeholder="Your cloud password">
        <button type="submit" name="action" value="submit_password" class="secondary">Submit password</button>
        """
